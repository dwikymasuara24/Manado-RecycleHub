...
try {
    ...
    if ($norm_q !== '') {
        $sql .= " OR pr.nomor_wa = :norm OR pr.nomor_wa LIKE :norm_like ";
        $params[':norm'] = $norm_q;
        $params[':norm_like'] = '%' . $norm_q . '%';
    }

    $sql .= " GROUP BY pr.id ORDER BY pr.created_at DESC LIMIT 20 ";

...
if ($wasteRows) {
    foreach ($wasteRows as $w) {
        $wasteLabels[] = $w['ikon_emoji'] . ' ' . $w['name'];
        $wasteWeights[] = (float)$w['total_kg'];
    }
}
?>
...
<div class="grid-2 mb-24">
  ...
  <div class="chart-card">
    <div class="card-title" style="margin-bottom: 20px;"><div class="ct-icon">📈</div> Tren Request per Minggu</div>
    <?php if ($weekData): ?>
      <div class="chart-container">
        <canvas id="chartMingguan"></canvas>
      </div>
    <?php else: ?>
      <p style="color:#aaa;text-align:center;padding:80px 0;font-size:13px">Tidak ada data tren mingguan</p>
    <?php endif; ?>
  </div>

  <!-- Distribusi Kecamatan -->
  <div class="chart-card">
    <div class="card-title" style="margin-bottom: 20px;"><div class="ct-icon">🏆</div> Distribusi per Kecamatan</div>
    <?php if ($kecRows): ?>
...
<div class="chart-card mb-24">
  <div class="card-title" style="margin-bottom: 20px;"><div class="ct-icon">🗑️</div> Jenis Sampah Terkumpul — <?= $bulanId[$month] ?> <?= $year ?></div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (ctxMingguan) {
        ...
        const counts = <?= json_encode($weekCounts) ?>;
    ...
    // --- 3. CHART WASTE CATEGORIES ---
    const ctxWaste = document.getElementById('chartWaste');
    if (ctxWaste) {
        const labels = <?= json_encode($wasteLabels) ?>;
        const weights = <?= json_encode($wasteWeights) ?>;
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        ...
                        callbacks: {
                            label: function(context) {
                                return ` ${context.parsed.x.toLocaleString('id-ID')} kg`;
                            }
                        }
                    }
                },

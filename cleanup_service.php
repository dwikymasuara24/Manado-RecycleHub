...
try {
    ...
    try {
        $existingCols = array_map('strtolower', $pdo->query("SHOW COLUMNS FROM `cleanup_requests`")->fetchAll(PDO::FETCH_COLUMN));
        if (!in_array('jam_kerja_aktual', $existingCols)) {
            $pdo->exec("ALTER TABLE `cleanup_requests` ADD COLUMN `jam_kerja_aktual` DECIMAL(10,2) NULL AFTER `estimasi_jam_kerja`");
        }
    ...
    $pdo->exec("CREATE TABLE IF NOT EXISTS `cleanup_items` (
        `id` bigint(20) UNSIGNED NOT NULL AUTO_INCREMENT,
        ...
        try {
            if ($is_code) {
                ...
                $st = $pdo->prepare("
                    ...
                    FROM   cleanup_requests cr
                    LEFT   JOIN officers o ON o.id = cr.officer_id
                    WHERE  cr.nomor_wa = :wa1
                        OR cr.nomor_wa LIKE :wa2
                        OR (:norm1 <> '' AND (cr.nomor_wa = :norm2 OR cr.nomor_wa LIKE :norm_like))
                    ORDER  BY cr.created_at DESC
                    LIMIT  20
                ");
                $st->execute([
            ...
            error_log('[MRH Cleanup Track] ' . $e->getMessage());
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['_action'] ?? '') === 'submit_form') {
    if (empty($_POST['service_type']))                   $errors[] = 'service_type';
    ...
    if (empty($errors)) {
        try {
            ...
            $svc = trim(clean($_POST['service_type']  ?? ''));
            $area = 0;
            $waste = trim(clean($_POST['dominant_waste'] ?? ''));
            ...
            if (!empty($_FILES['foto_lokasi']['name']) && $_FILES['foto_lokasi']['error'] === UPLOAD_ERR_OK) {
                ...
                if (in_array($ext, ['jpg', 'jpeg', 'png', 'webp'])) {
                    $upload_dir = PROJECT_ROOT . '/uploads/cleanup/';
                    if (!is_dir($upload_dir)) mkdir($upload_dir, 0777, true);
                    $foto_lokasi = 'loc_' . time() . '_' . rand(100, 999) . '.' . $ext;
                    move_uploaded_file($_FILES['foto_lokasi']['tmp_name'], $upload_dir . $foto_lokasi);
                }
            }
            ...
            $st = $pdo->prepare("
                INSERT INTO cleanup_requests
                    (request_code, service_type, nama_pemohon, nomor_wa, kecamatan, kelurahan, alamat_jemput,
                     latitude, longitude, area_size_sqm, dominant_waste, catatan, foto_lokasi,
                     estimasi_jam_kerja, biaya_estimasi, status, created_at, updated_at)
                VALUES
                    (:rc,:svc,:nm,:wa,:kec,:kel,:al,:lat,:lng,:area,:waste,:cat,:foto,
                     :jam,:biaya,'menunggu',NOW(),NOW())
            ");
            $st->execute([
                ':rc' => $request_code, ':svc' => $svc, ':nm' => $nm, ':wa' => $wa, ':kec' => $kec, ':kel' => $kel, ':al' => $al,
                ':lat' => $lat, ':lng' => $lng, ':area' => $area, ':waste' => $waste, ':cat' => $cat,
                ':foto' => $foto_lokasi,
                ':jam' => $estimasi_jam_kerja, ':biaya' => $biaya_estimasi
            ]);
            ...
            $sub_data = [
                ...
                'service'=> $cleanup_types[$svc]['label'] ?? $svc,
                'area'   => $area,
            ];
            ...
            error_log('[MRH Cleanup] ' . $e->getMessage());
        }
    }
}
...
$v = [
    ...
    'lng'     => $f('longitude'),
    'area'    => $f('area_size'),
    'waste'   => $f('dominant_waste'),
    'cat'     => $f('catatan'),
];

$init_step = 1;
if (!empty($errors)) {
    if (in_array('database_error', $errors)) $init_step = 4;
...
$status_label_map = [
    ...
    'sedang_cleanup'  => 'Sedang Dibersihkan',
    'selesai'         => 'Selesai',
...
$track_step_defs = [
    ...
    ['icon' => '🎉', 'label' => 'Selesai',      'desc' => 'Layanan selesai'],
];
?>
<!DOCTYPE html>
<html lang="id">
...
:root {
    ...
    --red:  #e53935; --blu:  #1976d2; --blul: #e3f2fd; --blum: #bbdefb; --td:   #1b2b1c; --tm:   #4a6649;
    --tl:   #7a9b7a; --wh:   #ffffff; --bg:   #f4faf4;
    --shad: 0 4px 24px rgba(46,125,50,.10); --rad:  16px;
...
.top-nav .site-name { font-size: 1rem; font-weight: 900; color: #fff; flex: 1; }
...
.cleanup-icon { font-size: 2rem; margin-bottom: 8px; }
.cleanup-name { font-weight: 800; font-size: .9rem; color: var(--gd); }
.cleanup-desc { font-size: .7rem; color: var(--tm); margin-top: 4px; }

.frow { display: flex; gap: 16px; margin-bottom: 16px; flex-wrap: wrap; }
.fw { flex: 1; min-width: 200px; }
.fw label { display: block; font-size: .78rem; font-weight: 800; color: var(--tm); text-transform: uppercase; margin-bottom: 6px; }
...
.req-code-box { display: inline-block; margin: 16px auto 0; background: rgba(255,255,255,.2); border: 2px solid rgba(255,255,255,.45); border-radius: 9px; padding: 9px 22px; font-size: 1.2rem; font-weight: 900; color: #fff; letter-spacing: .08em; font-family: monospace; }
...
.s-tl-node { width: 30px; height: 30px; border-radius: 50%; font-size: .72rem; font-weight: 900; display: flex; align-items: center; justify-content: center; flex-shrink: 0; }
...
.btn-track { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 14px 20px; background: var(--gd); color: #fff; border-radius: 11px; text-decoration: none; font-size: .92rem; font-weight: 800; transition: background .2s; }
.btn-track:hover { background: var(--gm); }
.btn-new { display: flex; align-items: center; justify-content: center; gap: 8px; padding: 12px 20px; background: transparent; color: var(--gm); border: 2px solid var(--gml); border-radius: 11px; text-decoration: none; font-size: .88rem; font-weight: 800; transition: background .2s, border-color .2s; }
...
.sbadge.sedang_cleanup  { background: #e0f2f1; color: #004d40; }
...
.track-detail span { display: flex; align-items: center; gap: 4px; }
...
<div class="tabs">
    <a href="?tab=form" class="tab-btn <?= $active_tab==='form'?'active':'' ?>">📝 Request Baru</a>
    <a href="?tab=track" class="tab-btn <?= $active_tab==='track'?'active':'' ?>">🔍 Lacak Layanan</a>
</div>

    <?php if ($active_tab === 'form'): ?>
        <?php if ($submitted): ?>
            <!-- Success Screen -->
                ...
                <div class="success-card">
                    ...
                    <div class="success-body">
                        ...
                        <div class="detail-recap">
                            ...
                            <div class="dr-row"><span class="dr-lbl">Layanan</span><span class="dr-val"><?= htmlspecialchars($sub_data['service'] ?? '') ?></span></div>
                        </div>
        ...
        <?php else: ?>
            <div class="page-wrapper">
                ...
                <aside class="sidebar-stepper" id="sidebarStepper">
                    ...
                    <?php
                    $ss_steps = [
                        ['icon'=>'🧹','label'=>'Layanan',      'desc'=>'Pilih kebutuhan'],
                        ['icon'=>'👤','label'=>'Identitas',    'desc'=>'Nama & kontak'],
                        ['icon'=>'📍','label'=>'Lokasi',       'desc'=>'Tentukan area'],
                        ['icon'=>'🧹','label'=>'Detail',       'desc'=>'Sampah & catatan'],
                ...
                <main class="form-main">
                    ...
                    <div class="top-stepper" id="topStepper">
                        ...
                        <div class="stepper-bar">
                            <div class="stp-node active" id="stp-node-1">1</div><div class="stp-line" id="stp-line-1"></div>
                            <div class="stp-node" id="stp-node-2">2</div><div class="stp-line" id="stp-line-2"></div>
                            <div class="stp-node" id="stp-node-3">3</div><div class="stp-line" id="stp-line-3"></div>
                            <div class="stp-node" id="stp-node-4">4</div><div class="stp-line" id="stp-line-4"></div>
                            <div class="stp-node" id="stp-node-5">5</div>
                        </div>
                        <div class="stepper-labels">
                            <div class="stp-lbl active" id="stp-lbl-1">Layanan</div>
                            <div class="stp-lbl" id="stp-lbl-2">Identitas</div>
                            <div class="stp-lbl" id="stp-lbl-3">Lokasi</div>
                            <div class="stp-lbl" id="stp-lbl-4">Detail</div>
                            <div class="stp-lbl" id="stp-lbl-5">Konfirmasi</div>
                        </div>
                    </div>
                    ...
                    <div class="form-main-inner">
                        <div class="card">
                            <form action="cleanup_service.php" method="POST" id="cleanupForm" enctype="multipart/form-data" novalidate>
                                <input type="hidden" name="_action" value="submit_form">

                                <!-- Step 1: Jenis Layanan -->
                                <div class="form-step active" id="step-1">
                                    <div class="step-title">🧹 Pilih Layanan</div>
                                    <div class="step-sub">Apa kebutuhan pembersihan Anda?</div>
                                    <div class="cleanup-grid">
                                        <?php foreach ($cleanup_types as $key => $t): ?>
                                        <label class="cleanup-item">
                                            <input type="radio" name="service_type" value="<?= $key ?>" <?= $v['service']===$key?'checked':'' ?>>
                                            <div class="cleanup-label">
                                                <div class="cleanup-icon"><?= $t['icon'] ?></div>
                                                <div class="cleanup-name"><?= $t['label'] ?></div>
                                ...
                                <div class="form-step" id="step-2">
                                    ...
                                    <div class="step-sub">Masukkan nama dan kontak yang bisa dihubungi.</div>
                                    <div class="frow">
                                        <div class="fw">
                                            <label for="nama">Nama Lengkap *</label>
                                            <input type="text" id="nama" name="nama" class="form-input" value="<?= htmlspecialchars($v['nama']) ?>" placeholder="Contoh: Budi Santoso">
                                        </div>
                                    </div>
                                    <div class="frow">
                                        <div class="fw">
                                            <label for="nomor_wa">Nomor WhatsApp *</label>
                                            <input type="text" id="nomor_wa" name="nomor_wa" class="form-input" value="<?= htmlspecialchars($v['wa']) ?>" placeholder="812xxxxxxx">
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="form-step" id="step-3">
                                    <div class="step-title">📍 Lokasi</div>
                                    <div class="step-sub">Tentukan lokasi area yang akan dibersihkan.</div>
                                            ...
                                            <div class="frow">
                                                <div class="fw">
                                                    ...
                                                    <select name="kecamatan" id="kecamatan" class="form-input">
                                                        ...
                                                        <?php foreach ($kec_opts as $k => $l): ?>
                                                        <option value="<?= $k ?>" <?= $v['kec']===$k?'selected':'' ?>><?= $l ?></option>
                                                        <?php endforeach; ?>
                                                    </select>
                                                </div>
                                                <div class="fw">
                                                    <label for="kelurahan">Kelurahan *</label>
                                                    <input type="text" name="kelurahan" id="kelurahan" class="form-input" value="<?= htmlspecialchars($v['kel']) ?>" placeholder="Nama kelurahan">
                                                </div>
                                            </div>
                                            <div class="fw">
                                                <label for="alamat_jemput">Alamat Lengkap *</label>
                                                <textarea name="alamat_jemput" id="alamat_jemput" class="form-input" rows="2" placeholder="Nama jalan, nomor rumah, atau patokan lokasi"><?= htmlspecialchars($v['al']) ?></textarea>
                                            </div>
                                            <div class="fw" style="margin-top:20px">
                                                <label for="foto_lokasi">Foto Lokasi (Opsional)</label>
                                                <input type="file" name="foto_lokasi" id="foto_lokasi" class="form-input" accept="image/*">
                                            </div>
                                        </div>
                                ...
                                <div class="form-step" id="step-4">
                                    <div class="step-title">🧹 Detail Area</div>
                                    <div class="step-sub">Berikan gambaran mengenai area Clean Up.</div>
                                    <input type="hidden" id="area_size" name="area_size" value="0">
                                    <div class="frow">
                                        <div class="fw">
                                            <label for="dominant_waste">Jenis Sampah Dominan</label>
                                            <input type="text" id="dominant_waste" name="dominant_waste" class="form-input" value="<?= htmlspecialchars($v['waste']) ?>" placeholder="Contoh: Botol Plastik, Daun, Material Bangunan">
                                        </div>
                                    </div>
                                    <div class="fw">
                                        <label for="catatan">Catatan Tambahan</label>
                                        <textarea name="catatan" id="catatan" class="form-input" rows="2" placeholder="Instruksi khusus untuk petugas..."><?= htmlspecialchars($v['cat']) ?></textarea>
                                    </div>
                                </div>
                                ...
                                <div class="form-step" id="step-5">
                                    ...
                                    <div class="step-sub">Tinjau kembali data Anda sebelum mengirim.</div>
                                    <div class="review-card" id="reviewRows">
                                        <!-- JS populated -->
                                    </div>
                                    <div style="background:var(--gl);padding:14px;border-radius:12px;font-size:.8rem;color:var(--gd);font-weight:700">
                                        ℹ️ Detail penugasan dan biaya layanan akan diinformasikan lebih lanjut oleh tim kami setelah verifikasi permintaan Anda.
                                    </div>
                                </div>
                            </form>
                        </div><!-- end .card -->
                        ...
                        <div class="card-footer" id="cardFooter">
                            <button type="button" class="btn-nav btn-prev" id="btnPrev" onclick="goStep(-1)" style="display:none">
                            ...
                            <button type="submit" form="cleanupForm" class="btn-nav btn-submit" id="btnSubmit" style="display:none">
                                Kirim Permintaan
    ...
    <?php elseif ($active_tab === 'track'): ?>
        <div class="track-wrap">
            <div class="track-search">
                <h2>🔍 Cek Status Layanan</h2>
                <p>Masukkan Order ID (contoh: CLN12345678) atau nomor WhatsApp Anda.</p>
                <form action="cleanup_service.php" method="GET" class="track-form">
                    <input type="hidden" name="tab" value="track">
                    <input type="text" name="q" value="<?= htmlspecialchars($track_query) ?>" placeholder="Contoh: CLN12345678 atau nomor WA..." style="flex:1" autocomplete="off" autofocus>
            ...
            <?php if ($track_query !== ''): ?>
                <?php if (empty($track_res)): ?>
                    <div class="track-empty">
                        ...
                        <div style="margin-top:10px;font-size:.82rem;color:var(--tl)">Pastikan Order ID atau nomor WA sudah benar.</div>
                    </div>
                <?php else: foreach ($track_res as $row): ?>
                <?php
                ...
                <div class="track-card">
                    ...
                    <div class="track-card-body">
                        ...
                        <div class="track-detail">
                            ...
                            <span>🧹 <?= htmlspecialchars($cleanup_types[$row['service_type']]['label'] ?? $row['service_type']) ?></span>

                            <?php if ($row['dominant_waste']): ?>
                            <span>♻️ Dominan: <?= htmlspecialchars($row['dominant_waste']) ?></span>
                            <?php endif; ?>
                        </div>
                        ...
                        <?php if (!empty($row['catatan_officer']) && in_array($row['status'], ['dalam_perjalanan', 'sedang_diproses', 'sedang_cleanup', 'selesai'])): ?>
                        ...
                        <?php if ($row['tanggal_tugas'] && in_array($row['status'], ['dijadwalkan','dalam_perjalanan','sedang_diproses','sedang_cleanup','selesai'])): ?>
                        <div class="track-schedule">
                            📅 Dijadwalkan: <strong><?= date('d M Y', strtotime($row['tanggal_tugas'])) ?></strong>
                            <?php if ($row['biaya_aktual'] !== null): ?>
                            <br>💵 Biaya Aktual: <strong>Rp <?= number_format($row['biaya_aktual'], 0, ',', '.') ?></strong>
                            <?php endif; ?>
                        </div>
                        <?php endif; ?>

                        <?php if (in_array($row['status'], ['dalam_perjalanan', 'sedang_diproses', 'sedang_cleanup'])): ?>
                        <div class="live-map-wrap" style="margin-top: 14px;">
                            ...
                            <div id="liveMap-<?= $row['id'] ?>" class="live-tracking-map" data-request-id="<?= $row['id'] ?>" data-request-type="cleanup" data-customer-lat="<?= floatval($row['latitude']) ?>" data-customer-lng="<?= floatval($row['longitude']) ?>" style="height: 250px; border-radius: 12px; border: 2px solid var(--gml); position: relative; z-index: 1;"></div>
                        </div>
                        <?php endif; ?>
...
function validateStep(step) {
    if (step === 1) {
        if (!document.querySelector('input[name="service_type"]:checked')) { alert('Pilih jenis layanan!'); return false; }
    }
    if (step === 2) {
        if (!document.getElementById('nama').value.trim()) { alert('Nama wajib diisi!'); return false; }
        if (!document.getElementById('nomor_wa').value.trim()) { alert('No. WA wajib diisi!'); return false; }
    }
    if (step === 3) {
        if (!document.getElementById('kecamatan').value) { alert('Kecamatan wajib dipilih!'); return false; }
        if (!document.getElementById('alamat_jemput').value.trim()) { alert('Alamat wajib diisi!'); return false; }
    }
    return true;
}
...
function buildReview() {
    ...
    const svc = document.querySelector('input[name="service_type"]:checked');
    const svcText = svc ? svc.closest('.cleanup-item').querySelector('.cleanup-name').innerText : '';
    const kec = document.getElementById('kecamatan');
    const kecText = kec.options[kec.selectedIndex].text;
    const latEl = document.getElementById('latitude');
    const lngEl = document.getElementById('longitude');
    const gpsTxt = (latEl && latEl.value && lngEl && lngEl.value)
        ? latEl.value + ', ' + lngEl.value : '—';

    const data = [
        ['Layanan', svcText],
        ['Nama', document.getElementById('nama').value],
        ['WhatsApp', document.getElementById('nomor_wa').value],
        ['Kecamatan', kecText],
        ['Alamat', document.getElementById('alamat_jemput').value],
        ['Koordinat GPS', gpsTxt]
    ];

    rows.innerHTML = data.map(r => `<div class="review-row"><span class="rl">${r[0]}</span><span class="rv">${r[1]}</span></div>`).join('');
}

// Google Maps Map logic
...
setTimeout(function() {
    if (window._google_map_instance) {
        google.maps.event.trigger(window._google_map_instance, 'resize');
        var latInput = document.getElementById('latitude');
        var lngInput = document.getElementById('longitude');
        if (latInput && lngInput && latInput.value !== '' && lngInput.value !== '') {
            var center = { lat: parseFloat(latInput.value), lng: parseFloat(lngInput.value) };
            window._google_map_instance.setCenter(center);
        }
    } else if (window._leaflet_map_instance) {
        window._leaflet_map_instance.invalidateSize();
        var latInput = document.getElementById('latitude');
        var lngInput = document.getElementById('longitude');
        if (latInput && lngInput && latInput.value !== '' && lngInput.value !== '') {
            var center = [parseFloat(latInput.value), parseFloat(lngInput.value)];
            window._leaflet_map_instance.setView(center, 16);
        }
    } else {
        // Auto fallback if Google maps didn't load in 3 seconds
        setTimeout(function() {
            if (!window._google_map_instance && !window._leaflet_map_instance) {
                window.initLeafletMapFallback();
            }
        }, 2700);
    }
